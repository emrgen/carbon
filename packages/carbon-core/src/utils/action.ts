...
export const insertNodesActions = (
  at: Point,
  nodes: Node[],
  origin: ActionOrigin = ActionOrigin.UserInput,
) => {
...
export const removeNodesActions = (
  nodes: Node[] | Node,
  origin: ActionOrigin = ActionOrigin.UserInput,
) => {
  const actions: CarbonAction[] = [];
...
export const insertBeforeAction = (
  ...
  const at = before.prevSibling
    ? Point.toAfter(before.prevSibling!.id)
    : Point.atOffset(before.parent!.id);
