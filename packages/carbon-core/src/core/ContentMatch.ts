...
export class ContentMatch {
	...
	static parse(string: string, nodeTypes: Record<NodeName, NodeType>): ContentMatch {
...
function parseExprSubscript(stream: TokenStream): Expr {
	let expr: Expr = parseExprAtom(stream);
	for (;;) {
		if (stream.eat("+"))
			expr = { type: "plus", expr }
		else if (stream.eat("*"))
...
function resolveName(stream: TokenStream, name: string): readonly NodeType[] {
	...
	for (let typeName in types) {
		...
		if (type.groupsNames.indexOf(name) > -1) result.push(type)
	}
...
function checkForDeadEnds(match: ContentMatch, stream: TokenStream) {
	for (let i = 0, work = [match];i < work.length;i++) {
		let state = work[i], dead = !state.validEnd;
